...
from utils.misc import load_from_checkpoint, save_checkpoint_extracted, get_optimizer

def train_test(cfg, classifier, trainloader, valloader, testloader, logger):
    device = classifier.parameters().__next__().device
    logger.info("TRAINING MODEL")
    ckpt, _ = train(classifier, trainloader, valloader, cfg, logger, device)
    logger.info("Training complete. Loading best model from checkpoint: {}".format(ckpt))
    _, classifier, _, metrics = load_from_checkpoint(cfg, device, ckpt)
    ...
    test(classifier, testloader, cfg, logger, device)



def train(classifier, trainloader, valloader, cfg, logger, device):
    ...
    for e in range(cfg.epochs):
        ...
        for i, samp in enumerate(trainloader):
            classifier.train()
            features, _, labels, _, lengths, _ = samp
            ...
            if cfg.val_interval and global_step % cfg.val_interval == cfg.val_interval - 1:
                ...
                harsh_metrics_val, lenient_metrics_val = test(classifier, valloader, cfg, logger, device)
                if harsh_metrics_val["rval"] > best_rval:
                    ...
                    checkpoint_path = save_checkpoint_extracted(classifier, optimizer, metrics, e+1)
...
def test(classifier, dataloader, cfg, logger, device):
    ...
    for samp in dataloader:
        features, segs, labels, _, lengths, _ = samp
        features = features.to(device).permute(2, 0, 1, 3)
